...
class Hamiltionian:
    ...
    def diag(self, psi, ik=0, debug=False):
        ...
        residual = np.maximum(residual, 1e-10)  # Avoid division by 0
    ...
    def scf(self, max_iter=50, beta=0.7, de_tol=1e-6):
        ...
        for i in range(max_iter):
            ...
            E = self.get_E_total()
            dE = abs(E - self.E_old)
            print(f"SCF{i:3d} dE:{dE:.8f} E_total:{E:.6f} drho:{d_rho:.6f}")
            print(self)
...
if __name__ == "__main__":
    ...
    # Planewave (3*3*3 grid with symmetry)
